...
/// Render `CONTEXT.md`
fn write_context_md(texts: &[TextFile], bins: &[BinaryFile], max: usize, out: &Path) -> Result<()> {
    let contents = render_context_md(texts, bins, max)?;
    write_file_atomically(out, contents).with_context(|| format!("write {}", out.display()))
}

fn render_context_md(texts: &[TextFile], bins: &[BinaryFile], max: usize) -> io::Result<Vec<u8>> {
    let mut f = Vec::new();
    md_header(&mut f, "# 📚 Project Context — generated by za")?;
...
fn download_github_snapshot(
    ...
    let mut cleanup = TempDirCleanup::new(root.clone());
